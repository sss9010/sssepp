            ...
            return (
              <AnimatedSection key={project.title} delay={0.1 + i * 0.08}>
                <article className="group h-full flex flex-col rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 hover:border-accent-200 dark:hover:border-accent-800/60 hover:shadow-md transition-all duration-300">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-2 group-hover:text-accent-700 dark:group-hover:text-accent-300 transition-colors leading-snug">
                    {project.title}
                  </h3>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed flex-1 mb-3">
                    {project.description}
                  </p>

                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {(project.methods ?? []).map((m) => (
                      <span key={m} className="px-2 py-0.5 rounded-md text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                        {m}
                      </span>
                    ))}
                  </div>
